...
namespace FlexAirFit.Tests
{
    public class ClientServiceUnitTests
    {
        ...
        {
            var client = new Client(Guid.NewGuid(), Guid.NewGuid(), "John Doe", "Male", DateOnly.FromDateTime(DateTime.Today), Guid.NewGuid(), DateOnly.FromDateTime(DateTime.Today), 10, false, new List<Tuple<DateOnly, DateOnly>>());
        ...
        public async Task DeleteClient_ShouldCallDeleteClientAsync_WhenClientExists()
        {
            ...
            _mockClientRepository.Setup(r => r.GetClientByIdAsync(clientId)).ReturnsAsync(new Client(clientId, Guid.NewGuid(), "John Doe", "Male", DateOnly.FromDateTime(DateTime.Today), Guid.NewGuid(), DateOnly.FromDateTime(DateTime.Today), 10, false, new List<Tuple<DateOnly, DateOnly>>()));
        ...
        {
            var clientId = Guid.NewGuid();
            var client = new Client(clientId, Guid.NewGuid(), "John Doe", "Male", DateOnly.FromDateTime(DateTime.Today), Guid.NewGuid(), DateOnly.FromDateTime(DateTime.Today), 10, false, new List<Tuple<DateOnly, DateOnly>>());
        ...
        public async Task GetClients_ShouldReturnListOfClients_WithLimitAndOffset()
        {
            ...
            {
                new Client(Guid.NewGuid(), Guid.NewGuid(), "John Doe", "Male", DateOnly.FromDateTime(DateTime.Today), Guid.NewGuid(), DateOnly.FromDateTime(DateTime.Today), 10, false, new List<Tuple<DateOnly, DateOnly>>()),
                new Client(Guid.NewGuid(), Guid.NewGuid(), "Jane Smith", "Female", DateOnly.FromDateTime(DateTime.Today), Guid.NewGuid(), DateOnly.FromDateTime(DateTime.Today), 8, false, new List<Tuple<DateOnly, DateOnly>>()),
                new Client(Guid.NewGuid(), Guid.NewGuid(), "Alex Johnson", "Male", DateOnly.FromDateTime(DateTime.Today), Guid.NewGuid(), DateOnly.FromDateTime(DateTime.Today), 12, true, new List<Tuple<DateOnly, DateOnly>>())
            };
        ...
        {
            var clientId = Guid.NewGuid();
            var client = new Client(clientId, Guid.NewGuid(), "John Doe", "Male", DateOnly.FromDateTime(DateTime.Today), Guid.NewGuid(), DateOnly.FromDateTime(DateTime.Today), 5, false, new List<Tuple<DateOnly, DateOnly>>());
            var freezingStart = DateOnly.FromDateTime(DateTime.Now);
        ...
        [Fact]
        public async Task CheckAndSetFreezingStatus_ShouldUpdateIsFreezing_WhenFreezingStatusNeedsToBeChanged()
        {
            var clientId = Guid.NewGuid();
            var client = new Client(clientId, Guid.NewGuid(), "John Doe", "Male", DateOnly.FromDateTime(DateTime.Today), Guid.NewGuid(), DateOnly.FromDateTime(DateTime.Today.AddDays(10)), 10, false, new List<Tuple<DateOnly, DateOnly>>
                {
                    new Tuple<DateOnly, DateOnly>(DateOnly.FromDateTime(DateTime.Now).AddDays(3), DateOnly.FromDateTime(DateTime.Now).AddDays(6))
                }
            );

            _mockClientRepository.Setup(r => r.GetClientByIdAsync(clientId)).ReturnsAsync(client);

            await _clientService.CheckAndSetFreezingStatus(clientId);

            Assert.False(client.IsFreezing);
            _mockClientRepository.Verify(r => r.UpdateClientAsync(client), Times.Never);
        }

        [Fact]
        public async Task CheckAndSetFreezingStatus_ShouldNotUpdateIsFreezing_WhenFreezingStatusDoesNotNeedToBeChanged()
        {
            var clientId = Guid.NewGuid();
            var client = new Client(clientId, Guid.NewGuid(), "John Doe", "Male", DateOnly.FromDateTime(DateTime.Today), Guid.NewGuid(), DateOnly.FromDateTime(DateTime.Today.AddDays(10)), 10, true, new List<Tuple<DateOnly, DateOnly>>
                {
                    new Tuple<DateOnly, DateOnly>(DateOnly.FromDateTime(DateTime.Now), DateOnly.FromDateTime(DateTime.Now).AddDays(5))
                }
            );
